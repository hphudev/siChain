...
const products_data = [
  {
    ...
    isRFID: 'Có',
    price: '4,600,000',
    export_date: '30/09/2022'
  }
]

mock.onGet('/api/datatables/products-data').reply(() => {
  return [200, products_data]
})
...
mock.onGet('/api/datatables/data-products').reply(config => {
  // eslint-disable-next-line object-curly-newline
  const { q = '', perPage = 10, page = 1 } = config
  /* eslint-enable */
  ...
  const filteredData = products_data.filter(
    item =>
      ...
      item.isRFID.toLowerCase().includes(queryLowered) ||
      item.price.toLowerCase().includes(queryLowered) ||
      item.export_date.toLowerCase().includes(queryLowered)
  )
  /* eslint-enable  */
